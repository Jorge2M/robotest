...
public class SecTarjetaPciInIframe extends PageObjTM implements SecTarjetaPci {
	...
	protected SecTarjetaPciInIframe(Channel channel, WebDriver driver) {
		super(driver);
		this.channel = channel;
	}
	
	public static SecTarjetaPciInIframe getNew(Channel channel, WebDriver driver) {
		return (new SecTarjetaPciInIframe(channel, driver));
	}
	...
	public void inputNumber(String number) {
		...
		if (number.compareTo(input.getAttribute("value").replace(" ", ""))!=0) {
			WebElement inputNumber = driver.findElement(By.xpath(XPathInputNumber));
			inputNumber.clear();
