...
import com.mng.robotest.tests.domains.ficha.pageobjects.nogenesis.tallas.SSecSelTallasFicha;
...
public class SecDataProduct extends PageBase {
	...
	private String getXPath(ColorType colorType) {
		switch (colorType) {
		case SELECTED:
			return "//div[@class[contains(.,'color-container--selected')]]";
		case LAST:
			return XP_COLOR + "[last()]";
		case AVAILABLE:
			return XP_COLOR + "/img[not(" + CLASS_COLOR_NO_DISP + ")]/..";
		case UNAVAILABLE: 
		default:
			return XP_COLOR + "/img[" + CLASS_COLOR_NO_DISP + "]/..";			
		}
	}
...
//Funciones referentes a los colores
	...
	private List<WebElement> getListColors() {
		return getElements(getXPath(ColorType.AVAILABLE));
	}

	public String getCodeColor(ColorType colorType) {
		var color = getElementWeb(getXPath(colorType));
	...
	public List<String> getColorsGarment() {
		...
		for (var element : getElements(XP_COLORES_PRENDA_SIN_IDENTIFICAR)) {
