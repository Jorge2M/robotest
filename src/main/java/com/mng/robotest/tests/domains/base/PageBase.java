...
public class PageBase extends PageObjTM {
	...
	protected boolean isCheckeableNewCheckout() {
		return !(isPRO() && UtilsTest.todayBeforeDate("2024-11-15"));
	}
	...
	protected boolean isVisibleCenterInScreen(String xpath) {
		...
		var centerX = elem.getRect().getWidth() / 2;
		var centerY = elem.getRect().getHeight() / 2;
		return isVisibleInScreen(elem, centerX, centerY);		
	}
