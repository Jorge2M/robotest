...
public abstract class PageGaleriaDevice extends PageGaleriaNoGenesis {
	...
	public void clickHearhIcon(int posArticle) throws Exception {
		moveToHearthIcon(posArticle);
		clickHearthIconAndWait(posArticle);
	}
	
	private void moveToHearthIcon(int posArticle) {
		moveToArticle(posArticle);
		var icon = getElement(getXPathArticleHearthIcon(posArticle));
		state(VISIBLE, icon).wait(1).check();
		moveToElement(icon);
	}
	
	private void clickHearthIconAndWait(int posIcon) throws Exception {
		var estadoInicial = getStateHearthIcon(posIcon);
		clickHearthIconPreventingOverlapping(posIcon);
		waitToHearthIconInState(posIcon, estadoInicial.getOpposite(), 2);
	}	
	...
	private void clickHearthIconPreventingOverlapping(int posIcon) throws Exception {
		var icon = getElement(getXPathArticleHearthIcon(posIcon));
		try {
			clickHearthIcon(icon);
		}
